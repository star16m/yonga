...
//	@Scheduled(initialDelay = 3 * 60 * 1000, fixedDelay = 15 * 60 * 1000)
    ...
    public @ResponseBody
    String extract(@RequestBody Map<String, Object> optionMap) {
    ...
    private void extract(List<Category> categoryList, ExtractMode extractMode) {
        ...
        if (extractMode.isRequiredInitialize())
            EXECUTOR.submit(new DataCleanWorker(this.categoryService, this.productService, this.siteInfo, this.logService, categoryList));
        EXECUTOR.submit(new DataExtractWorker(this.categoryService, this.siteInfo, this.logService, this.configService, categoryList, extractMode, this.mailService,
                productRepository, productImageRepository, makerRepository, brandRepository, keijoRepository, objectMapper, this.gson));
    }
}
